document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NEW: ADVANCED EXCEL EXPORT (Corrected) ---
    function saveAsExcel(isFullProject = true) {
        if (!isFullProject) { exportFilteredToExcel(); return; }
        const wb = XLSX.utils.book_new();
        let grandTotal = 0;
        let currentScheduleBreak = "N/A";
        const panelItems = getPanelItems();
        if (panelItems.length === 0) { alert("No sequences to export."); return; }
        panelItems.forEach(pItem => {
            if (pItem.type === 'schedule_break') { currentScheduleBreak = pItem.name; } 
                ...
                if (breakdownsInSequence.length > 0) {
                    ...
                    XLSX.utils.book_append_sheet(wb, ws, pItem.name.replace(/[/\\?*:[\]]/g, '').substring(0, 31));
                }
            }
        });
        if (currentMode === 'producer' && wb.SheetNames.length > 0) { const summaryData = [['GRAND TOTAL', grandTotal]]; const summaryWs = XLSX.utils.aoa_to_sheet(summaryData); XLSX.utils.book_append_sheet(wb, summaryWs, "Total"); }
        if (wb.SheetNames.length === 0) { alert("No breakdowns to export."); return; }
        XLSX.writeFile(wb, `${(projectData.projectInfo.prodName || 'Full_Project_Breakdown')}.xlsx`);
    }

    // This is the single most important line that was missing.
    initialize();
});
